...
everything is fine the line is the model with its live effort level, the working
folder, the five-hour quota with its reset time and the context percentage, with
`/1M` when the window is the extended one (quota and context always visible: they
are your margin). The full
