...
- **Prompt-audit nativo** (`/doctor prompt-audit fable-director`): un primo giro `claude -p` è scaduto a 15 minuti senza rapporto (48 letture, 6 comandi negati in dontAsk); il secondo è stato fermato per il render. Eseguito in sessione interattiva il 26/09 alle 09:00: 7 hunk applicati (`docs/prompt-audit-2026-09-26.md`), tra cui il README della scheda che diceva di lanciare a mano la statusline che si installa da sola, e la skill `verifica-modello` che citava le cartelle account dell'autore.
