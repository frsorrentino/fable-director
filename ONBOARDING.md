...
- Opting out: set `"autoUpdate": false` under `extraKnownMarketplaces.fsorrentino` in
...
2. MERGE-edit, preserving every existing key:
   - In "extraKnownMarketplaces": if a "fsorrentino" entry with a "directory"
     source already exists, replace it; otherwise add:
       "fsorrentino": {
         "source": { "source": "github", "repo": "frsorrentino/fable-director" },
         "autoUpdate": true
       }
   - In "enabledPlugins" add: "fable-director@fsorrentino": true
