...
|---|---|---|---|
| 1 | `video-sheet.sh <file> [--fps 1 \| --scenes 0.3] [--cols 8] [--width 400] [--out DIR]` | zero | ffprobe card + contact sheet with burnt-in timestamps; 68 s → one 8×9 sheet the top model reads in one `Read`. Above 90 frames it splits into numbered sheets. `--scenes 0.3` = one frame per cut plus the cut times (68 s spot → 19 cuts, one 8×3 sheet = the shot list); 0.15 also catches soft transitions. 320 px or 1 frame / 2 s loses small on-screen text. Cached per file hash: same clip later = a copy. |
| 2 | `transcribe.py <file> [--model small] [--lang it] [--out F.srt] [--json] [--words]` | zero (local CPU) | faster-whisper `small` int8: 68 s of Italian speech in 76 s (1.1× realtime), first model download 63 s; `--words` adds word timestamps to the JSON and doubles the time. Refuses files without an audio track BEFORE loading the model. `medium` (~1.5 GB) announces its download. Cached per file hash: same clip later = 0 s, no venv needed. |
...
1. **`video-sheet.sh` on every file.** Read the `audio:` line first; it decides step 2. Add `--scenes 0.3` when the deliverable is a shot list: the `scenes:` line gives the cut times and the sheet shows the first frame of each shot. Keep the sheets: they are the evidence for every timestamp you quote.
...
6. **Deliverable**: scene table + work table with one letter per item — **G** shot material (exists, reusable), **M** motion graphics to make, **V** voice-over, **S** stock — plus the timing table from step 3 when a script exists. Work table layout, one row per script block: `# | block (slot) | what it asks | what exists (timestamps) | what is missing | letters`, closed by a count line (blocks with new text, with new voice, with no usable footage). Estimate table, when asked: `item | basis of the count (from the tables above) | hours`.
...
- Setup: the plugin ships the scripts, not the local pieces. `media-doctor.py` lists what is missing (system `ffmpeg`/`ffprobe`, the venv with faster-whisper + edge-tts, the `gemini-media` provider); `media-doctor.py --setup [--prefetch small]` creates the venv and installs the packages, printing every command (~370 MB, plus ~470 MB for the `small` model). Any missing piece is reported as `STATUS: unavailable` — say it to the user and stop, do not read the video "by eye" without saying so.
