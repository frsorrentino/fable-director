...
- Code-writing contracts carry a standard Constraints line: *minimum code that passes Verification; reuse > stdlib > native platform feature > installed dependency > new code; no unrequested abstractions; ties go to the option correct on edge cases*. Known edge cases (inflections, boundaries, empty inputs) belong in the contract explicitly — what Verification doesn't test doesn't exist, and unrequested care is variance, not a guarantee (measured: A/B ponytail 2026-08, stem-matching case). The executor's escape valve for out-of-contract value is a `PROPOSAL:` line in its report, never silent code — the director decides.
