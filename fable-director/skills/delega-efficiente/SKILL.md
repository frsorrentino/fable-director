...
6. **Cache locality** — every subagent pays a COLD START: own uncached prefix, re-fed context (it does NOT break the main thread's cache); switching model or editing plugins/skills mid-session invalidates the prompt cache. Before spawning: does the cold start cost more than the delegation saves? Topology follows: one workflow with grouped items ≫ N single-item subagents (one warm prefix vs N cold starts; siblings launched in the same message share the system prefix and pay about half the cold start — measured 2026-09-01: 29k create for the first, 13k create + 16k read for the next). FORK ≠ fresh subagent: the fork inherits conversation + prompt cache (measured n=3, 2026-09-01, CC 2.1.257: per-turn cache_create 200-2.000 tok, cache_read = the whole ~150k parent context, model override ignored) but re-pays the whole parent context every turn at cache_read rates, always on the parent's model, and its spend is nearly invisible to the 2×/3× gate (fresh-token accounting excludes cache_read; eq accounting captures it ex post, per model). The parity point is MODEL-CONDITIONAL (`model-economics.json`): on Claude Fable 5.1 (cache reads 0.025×) a fork turn at 150k CTX costs ~4-5k eq against a 17-37k eq cold start → fork pays off up to ~7-12 turns; on 0.1×-cache models the same turn costs ~15k eq → ~2 turns. Fork for short, context-heavy tasks no spec can bundle; batch/spec-able work stays on fresh pinned executors (measured n=3 real items: `fd-executor` at low effort on sonnet vs on Fable 5.1 — same eq, same correctness, 5.5× the USD on Fable; the executor stays on sonnet). Cost VETO on borderline axis-4/5 routes — never forces delegation, never overrides axes 1-2.
...
- Every delegation prompt is a 5-part spec contract — Objective / Files (exact paths in scope) / Interfaces (output format + status token) / Constraints (tools, sources, boundaries, hard caps) / Verification (the command or check the subagent must run and report actual output of). Vague delegation ("occupati di X") duplicates work — forbidden; a spec the subagent can execute without shared context is the test that the route is delegable at all. For GPT/Codex-family executors (external route) wrap the 5 parts in XML blocks with stable tag names (`<task>`, `<output_contract>`, `<default_follow_through_policy>`, `<grounding_rules>`): block contracts hold better than prose on that family (distilled from openai/codex-plugin-cc); tighten the contract before raising effort. The contract's measured falsifier is **rework**: files reopened after their first write (auto-counted by the Stop hook and `report`) mean information landed AFTER the code was written — the context bundle was incomplete, not the executor weak. A task type with recurring reopens is a **context-pack candidate**: a deterministic script that gathers files/state/schemas BEFORE delegation, so the spec ships the whole picture instead of letting anyone discover it piecemeal.
- Code-writing contracts carry a standard Constraints line: *think longer to write only what's needed — less redundancy, more quality, never brevity for its own sake; reuse > stdlib > native platform feature > installed dependency > new code; no unrequested abstractions; ties go to the option more correct on edge cases and more efficient, never merely shorter*. Known edge cases (inflections, boundaries, empty inputs) belong in the contract explicitly — what Verification doesn't test doesn't exist, and unrequested care is variance, not a guarantee (measured: A/B ponytail 2026-08, stem-matching case). The executor's escape valve for out-of-contract value is a `PROPOSAL:` line in its report, never silent code — the director decides.
...
- Interfaces in practice: hard cap (default ~1-2k tokens), forced schema (strict JSON or grep-able `path:line — finding` lines), full-content dumps forbidden — paths, counts, anomalies only. The subagent's output is the next turn's input: a token the orchestrator won't consume is waste. The INPUT side has the same discipline: when the executor only CONSUMES an API (calls it, never edits it), Files ships its surface — `tools/skeleton.py FILE` prints signatures with file:line at ~5% of the file's tokens — not the full file; fresh input per agent is the dominant delegation cost.
...
This writes `~/.claude/fable-director/budgets/<cwd-slug>.json`. The plugin's Stop hook compares actual tokens (since declaration, from the main transcript PLUS the Workflow tool's agent files — those never surface in the main transcript and were invisible until measured at 9.2M fresh input in one session) at every turn end: at ≥2× it warns ONCE (checkpoint: reassess the route now — a reversal at 2× is cheaper than a post-mortem at 3×); at ≥3× it BLOCKS closure and demands the post-mortem. Thresholds on consumed tokens only, never on self-estimated progress.
