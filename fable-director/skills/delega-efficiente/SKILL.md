...
6. **Cache locality** — every subagent pays a COLD START: own uncached prefix, re-fed context (it does NOT break the main thread's cache); switching model or editing plugins/skills mid-session invalidates the prompt cache. Before spawning: does the cold start cost more than the delegation saves? Topology follows: one workflow with grouped items ≫ N single-item subagents (one warm prefix vs N cold starts). FORK ≠ fresh subagent: the fork inherits conversation + prompt cache — no 17k cold start (measured n=3, 2026-08-21: per-turn cache_create 106-696 tok, cache_read ≈ the whole 105-137k parent context) — but re-pays the whole parent context every turn at cache_read rates, always on the parent's model (override ignored), and its spend is nearly invisible to the 2×/3× gate (fresh-token accounting excludes cache_read; eq accounting captures it ex post). Fork for short, context-heavy tasks no spec can bundle: pays off up to ~3-5 turns at ~100-140k CTX, ~1-2 beyond 200k (per-turn term measured, parity derived from the 21k-eq cold-start and 6.5k-eq/turn anchors); batch/spec-able work stays on fresh pinned executors. Cost VETO on borderline axis-4/5 routes — never forces delegation, never overrides axes 1-2.
