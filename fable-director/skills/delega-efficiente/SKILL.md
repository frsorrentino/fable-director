...
- SEO / migrations / imports → axis 4. Verify: assertion ladder. Non-code batch items may route to a free external executor (`scripts/external-exec.py` — Gemini/Codex via cross-family config, zero Claude tokens, logs `external_exec`, built-in JSON rung-1): EXPERIMENTAL, per-case choice until `report` shows a DENSE ok-rate for that type; axis 2 items never take this route.
