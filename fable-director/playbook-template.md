...
- [seed 2026-09] workflow multi-ondata al muro 5h: il costo del muro = larghezza in volo × costo per agente (16 giudici Fable = 1,5-2 finestre: non finisce MAI in una finestra); resume = solo prefisso invariato (args oggetto, niente date negli stadi iniziali, script editato solo dopo l'ultima chiamata cachata); lanciatore magro (600k di contesto = ~720k eq a ogni ripresa dopo il muro); model/effort per stadio (lettori e verificatori su sonnet, top model solo per il giudizio); PDF scansionati → txt una volta. Misurato: run wf_4798f715 del 03/09, 18,8M eq totali, ~70% spreco; dal plugin 1.40.0 il gate fa l'aritmetica (window fit, resume hygiene, lint) (uses:0 ok:0 ko:0)
