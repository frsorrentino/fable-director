...
| `scripts/cross-verify.py` | Cross-family verifier (ladder rung 4): adversarial check by a different model family (Gemini/Codex CLI), out of Claude quota, no silent fallback — `unavailable` is never "verified". Config `~/.claude/fable-director/cross-family.json` (`--init`) |
