...
| `scripts/subagent-meter.py` (SubagentStart/SubagentStop hooks) | Counts delegations that actually started — nested spawns included, which the PreToolUse gate never sees (Claude Code ≥ 2.1.219 nests 3 levels by default) — and compares each subagent's real `effort.level` against the tier pinned in its frontmatter, logging `effort_ignored` when the pin was ignored. State per session in `~/.claude/fable-director/subagents/`, read by the statusline (`dlg ⟲N`) and `/fable-director:status`. Lock + atomic rename (a workflow fan-out starts many at once); any internal error exits 0 in silence — a meter must never break the session it measures |
