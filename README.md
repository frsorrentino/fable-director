...
![version](https://img.shields.io/badge/version-1.43.1-blue) ![license](https://img.shields.io/badge/license-MIT-green) ![Claude Code](https://img.shields.io/badge/Claude%20Code-plugin-8A5CF6)
...
`maxEffortLevel` (Claude Code ≥ 2.1.267) caps every request, pinned agents
included — measured: with `maxEffortLevel: medium` the verifier pinned `high`
ran at `medium` and `subagent-meter` logged `effort_ignored`. Keep the cap at
or above `high`, or set it per model under `modelSettings` so the verifier's
model keeps its tier.
